...
- Xcode's system-type color is applied by **name**, not by resolution. A project type whose name matches any SDK type, including nested ones such as SwiftUI's `Kind` or Foundation's `Category`, is painted purple even when the project's own declaration wins lookup. Sample code must avoid such names. `sample/Landmarks` uses `Place` for this reason; `Kind` and `Category` both collide.
...
- The theme file in the dotfiles is itself a symlink into the `vendor/xcode-system-theme` submodule. Zed watches the themes folder, not the symlink target, so a `git pull` in the submodule changes nothing on screen. Re-create the symlink (`rm` then `ln -s` again) and Zed reloads within a few seconds; a restart also works.
...
- Capture by compositing, never by a full-screen grab. `screencapture -l <id>` without `-o` returns the window with its shadow as alpha. The shadow depends on whether the window is active: 46 px left and right, 32 above, 60 below at 2x when inactive, and 112, 76, and about 150 when it is the key window. Since windows are raised before capture, the capture's opaque bounding box (`magick -alpha extract -threshold 99% -format %@`) is measured and the window placed at grid position times two minus that offset. A constant offset put every window down and to the right and clipped the bottom row. The composite is written at 2x as `hero-{mode}@2x.png` and halved for the README's `hero-{mode}.png`, which links to the 2x file. GitHub strips `target` attributes, so the link cannot force a new tab.
...
- Every window must be unobscured when captured. macOS 27 glass toolbars, Xcode's especially, are backdrops the compositor only renders while the window is unobscured; a window captured from behind another comes back with dither-noise blocks where its toolbar items sit. Being inactive is fine. `capture` activates the apps in a rotation so each grid window is raised but inactive at its capture, and makes the Ghostty demo window key last, so only the terminal shows lit traffic lights. Activating Ghostty makes its most recent window key, which is this session's terminal, hence the explicit `AXRaise` on the demo window.
...
- `sample/Landmarks/.zed/settings.json` excludes `.build`, `.swiftpm`, and `.zed` from Zed's tree, since sourcekit-lsp keeps recreating an index build there.
- Xcode's canvas state is restored per window, so check the Canvas menu item's mark and click until it reads `missing value`.
- Sample output must not show git history or `ls -l` owner columns; both leak the account name. Zed ignores `git.inline_blame` in a project's `.zed/settings.json`, so `hero.sh capture` switches it off in the user's `settings.json` for the run and restores the file on exit; Zed applies the change live within a few seconds. The Finder sidebar still shows the home folder name.
