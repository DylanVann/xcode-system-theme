...
#
#   tools/hero/hero.sh setup       open and size the four windows on sample/Landmarks
...
ghostty_window() {
  ...
  osascript -e "$SE to keystroke \"cd $PKG; clear; bash $S/tools/hero/demo.sh\"" -e "$SE to key code 36"; sleep 12
...
finder_window() {
  # Finder may still be relaunching after a killall, so retry until the window exists.
  local i; for i in $(seq 1 20); do
    ...
    end tell" >/dev/null 2>&1 && sleep 1 && [ -n "$(win Finder Landmarks)" ] && return 0
    sleep 1
  done
  echo "could not open the Finder window"; return 1
}
...
capture_mode() {
  ...
  # Put Zed's cursor back at line 1 so no current-line highlight or breadcrumb path shows,
  # and park the pointer in the gap between the windows so no hover effect ends up in the shot.
  /Applications/Zed.app/Contents/MacOS/cli "$FILE:1:1" >/dev/null 2>&1 || true
  $WM $(( X1 - 8 )) $(( Y1 + 39 - 24 )); sleep 1
